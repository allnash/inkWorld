...
import {
  Pen, MousePointer, Eraser, Hand, Undo2, Redo2, Bug, Droplet,
  FilePlus, Sparkles, Box as BoxIcon, Package, Trash2, RotateCw, Eye, X, LayoutGrid, PanelRightOpen, PanelRightClose,
} from 'lucide-react';
import { Toaster, showToast } from './toast/Toast';
import { ThreeViewer } from './components/ThreeViewer';
import { AssetPanel } from './components/AssetPanel';
import { FloorView } from './components/FloorView';
import { generateThreeJSScene, generateThreeJSSceneFromText, isAnthropicConfigured } from './ai/AnthropicService';
import type { SceneObject } from './ai/AnthropicService';
import { loadAssets, saveAsset, loadFloor, saveFloor, generateAssetId, generateFloorItemId } from './services/AssetStore';
import type { SavedAsset, FloorItem } from './services/AssetStore';
import './App.css';
...
function App() {
  ...
  // Three.js 3D viewer state
  const [sceneObjects, setSceneObjects] = useState<SceneObject[]>([]);
  const [sceneDescription, setSceneDescription] = useState<string>('');
  const [isGenerating3D, setIsGenerating3D] = useState(false);
  const inkCanvasElementRef = useRef<HTMLCanvasElement | null>(null);

  const handleCanvasReady = useCallback((canvas: HTMLCanvasElement) => {
    inkCanvasElementRef.current = canvas;
  }, []);

  // Text prompt for text-to-3D generation
  const [textPrompt, setTextPrompt] = useState('');

  const handleGenerate3D = useCallback(async () => {
    console.log('[3D] Generate clicked, canvas ref:', !!inkCanvasElementRef.current);
    if (!inkCanvasElementRef.current) {
      showToast('Canvas not ready');
      return;
    }
    if (!isAnthropicConfigured()) {
      showToast('OpenRouter API key not set — add INK_OPENROUTER_API_KEY to .env and restart dev server');
      return;
    }
    setIsGenerating3D(true);
    try {
      console.log('[3D] Rasterizing canvas...');
      const dataUrl = inkCanvasElementRef.current.toDataURL('image/png');
      console.log('[3D] Sending to Claude via OpenRouter...');
      const scene = await generateThreeJSScene(dataUrl);
      console.log('[3D] Got scene:', scene.description, scene.objects.length, 'objects');
      setSceneObjects(scene.objects);
      setSceneDescription(scene.description);
    } catch (err) {
      console.error('[3D] Error:', err);
      showToast('Failed to generate 3D: ' + (err as Error).message);
    } finally {
  ...
  const handleGenerateFromText = useCallback(async () => {
    ...
    setIsGenerating3D(true);
    try {
      console.log('[3D] Generating from text:', textPrompt);
      const scene = await generateThreeJSSceneFromText(textPrompt.trim());
      console.log('[3D] Got scene:', scene.description, scene.objects.length, 'objects');
      setSceneObjects(scene.objects);
      setSceneDescription(scene.description);
    } catch (err) {
  ...
  // Asset library & floor state
  const [assets, setAssets] = useState<SavedAsset[]>(loadAssets);
  const [assetPanelOpen, setAssetPanelOpen] = useState(false);
  const [showSaveDialog, setShowSaveDialog] = useState(false);
  const [saveAssetName, setSaveAssetName] = useState('');
  const [centerView, setCenterView] = useState<'canvas' | 'floor'>('canvas');
  const [threePanelOpen, setThreePanelOpen] = useState(true);
  const [floorItems, setFloorItems] = useState<FloorItem[]>(loadFloor);
  const [selectedFloorItem, setSelectedFloorItem] = useState<string | null>(null);
  ...
  const handleSaveAsset = useCallback(() => {
    if (!saveAssetName.trim() || sceneObjects.length === 0) return;
    saveAsset({
      id: generateAssetId(),
  ...
  }, [saveAssetName, sceneObjects, sceneDescription, refreshAssets]);

  const handleLoadAsset = useCallback((asset: SavedAsset) => {
    setSceneObjects(asset.objects);
    setSceneDescription(asset.name);
  }, []);

  const handleFloorItemsChange = useCallback((items: FloorItem[]) => {
    setFloorItems(items);
    saveFloor(items);
  }, []);

  const handleDropAssetOnFloor = useCallback((assetId: string, x: number, z: number) => {
    const asset = assets.find(a => a.id === assetId);
    if (!asset) return;
    const newItem: FloorItem = {
      id: generateFloorItemId(),
      assetId,
      name: asset.name,
    ...
    const newItems = [...floorItems, newItem];
    setFloorItems(newItems);
    saveFloor(newItems);
    setSelectedFloorItem(newItem.id);
  }, [assets, floorItems]);

  // Build combined scene from floor items for 3D viewer
  const floorSceneObjects = useMemo<SceneObject[]>(() => {
    // Add a floor plane
    ...
    const itemObjects: SceneObject[] = floorItems.flatMap(item => {
      const s = item.scale ?? 1;
      return item.objects.map(obj => ({
        ...obj,
        position: [
          obj.position[0] * s + item.x,
          obj.position[1] * s + (item.y ?? 0),
          obj.position[2] * s + item.z,
        ] as [number, number, number],
        rotation: [
          obj.rotation[0],
          obj.rotation[1] + item.rotation,
          obj.rotation[2],
        ] as [number, number, number],
        scale: [
          obj.scale[0] * s,
    ...
    return [floor, ...itemObjects];
  }, [floorItems]);

  const handleViewFloorIn3D = useCallback(() => {
    setSceneObjects(floorSceneObjects);
    setSceneDescription(`Room with ${floorItems.length} items`);
    setCenterView('canvas'); // switch back to see 3D result
  }, [floorSceneObjects, floorItems.length]);
  ...
  return (
    <div className="app">
      ...
      <nav className="left-sidebar">
        ...
        <button className="sidebar-btn" onClick={handleNewNote} title="New canvas">
          <FilePlus size={20} />
        </button>

        <div className="sidebar-divider" />

        <button className={`sidebar-btn ${currentTool === 'pen' ? 'active' : ''}`} onClick={() => setCurrentTool('pen')} title="Pen (P)">
          <Pen size={20} />
        </button>
        <button className={`sidebar-btn ${currentTool === 'select' ? 'active' : ''}`} onClick={() => setCurrentTool('select')} title="Select (S)">
          <MousePointer size={20} />
        </button>
        <button className={`sidebar-btn ${currentTool === 'eraser' ? 'active' : ''}`} onClick={() => setCurrentTool('eraser')} title="Eraser (E)">
          <Eraser size={20} />
        </button>
        <button className={`sidebar-btn ${currentTool === 'pan' ? 'active' : ''}`} onClick={() => setCurrentTool('pan')} title="Pan (H)">
          <Hand size={20} />
        </button>

        <div className="sidebar-divider" />

        <button className="sidebar-btn" onClick={undo} disabled={!canUndo} title="Undo">
          <Undo2 size={20} />
        </button>
        <button className="sidebar-btn" onClick={redo} disabled={!canRedo} title="Redo">
          <Redo2 size={20} />
        </button>

        <div className="sidebar-divider" />

        <button className={`sidebar-btn ${showDebug ? 'active' : ''}`} onClick={() => setShowDebug(d => !d)} title="Debug">
          <Bug size={20} />
        </button>

        <div className="sidebar-divider" />

        <button className={`sidebar-btn ${assetPanelOpen ? 'active' : ''}`} onClick={() => setAssetPanelOpen(o => !o)} title="Assets">
          <Package size={20} />
        </button>
        <button
          className={`sidebar-btn ${centerView === 'floor' ? 'active' : ''}`}
          onClick={() => setCenterView(v => {
            if (v === 'floor') {
              return 'canvas';
            } else {
              setThreePanelOpen(false);
              return 'floor';
            }
          })}
          title="Floor Plan"
        ...
        <button
          className={`sidebar-btn ${threePanelOpen ? 'active' : ''}`}
          onClick={() => setThreePanelOpen(o => !o)}
          title={threePanelOpen ? 'Hide 3D Panel' : 'Show 3D Panel'}
        >
          {threePanelOpen ? <PanelRightClose size={20} /> : <PanelRightOpen size={20} />}
        </button>

        <div className="sidebar-spacer" />

        {/* Colors & Brush */}
        <div className="sidebar-colors" style={{ opacity: drawingControlsEnabled ? 1 : 0.4 }}>
          ...
          <div className="color-picker-item" title="Fill color">
            <input
              type="color"
        ...
        <div className="sidebar-brush-size" style={{ opacity: drawingControlsEnabled ? 1 : 0.4 }}>
          <input
            type="range"
            min="1"
            max="20"
          ...
          <>
            ...
            <div className="canvas-header">
              ...
              <div className="canvas-header-info">
                ...
                <span>P/S/E/H: Tools</span>
              </div>
            </div>

            {hasSelectedSketchImage && (
              <div className="toolbar-secondary-float">
        ...
        {centerView === 'floor' && (
          <>
            <div className="canvas-header">
              <h1>Floor Plan</h1>
              <div className="canvas-header-info">
                <span>{floorItems.length} items placed</span>
                {floorItems.length > 0 && (
                  <button
                    onClick={handleViewFloorIn3D}
                    style={{
                      padding: '3px 10px',
            ...
            <div className="floor-view-fullscreen">
              <FloorView
                items={floorItems}
                onItemsChange={handleFloorItemsChange}
                selectedItemId={selectedFloorItem}
                onSelectItem={setSelectedFloorItem}
                onDropAsset={handleDropAssetOnFloor}
              />
            </div>
          </>
        )}
      ...
      {assetPanelOpen && (
        <AssetPanel
          ...
          onDragAsset={() => {}}
          onPlaceAsset={centerView === 'floor' ? (asset) => {
            handleDropAssetOnFloor(asset.id, 0, 0);
          } : undefined}
      ...
      {/* ---- Right: 3D Panel ---- */}
      {threePanelOpen && <div className="three-panel">
        <div className="three-panel-prompt">
          <input
            type="text"
            className="three-prompt-input"
            placeholder="Describe an object... (e.g. wooden desk with monitor)"
            value={textPrompt}
            onChange={e => setTextPrompt(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !isGenerating3D) handleGenerateFromText(); }}
            disabled={isGenerating3D}
          />
          <button
            className="three-prompt-btn"
            onClick={handleGenerateFromText}
            disabled={isGenerating3D || !textPrompt.trim()}
          >
            {isGenerating3D ? 'Generating...' : 'Generate'}
          </button>
        </div>
        <div className="three-panel-toolbar">
          <button onClick={handleGenerate3D} disabled={isGenerating3D} title="Generate 3D from canvas drawing">
            From Sketch
          </button>
          ...
          {sceneObjects.length > 0 && (
            <button onClick={() => { setSceneObjects([]); setSceneDescription(''); }}>
              Clear
            </button>
          )}
        </div>
      ...
      {showSaveDialog && (
        <div className="save-dialog-overlay" onClick={() => setShowSaveDialog(false)}>
          <div className="save-dialog" onClick={e => e.stopPropagation()}>
            ...
            <input
              type="text"
              placeholder="Asset name (e.g. Desk, Couch, Tree)"
              value={saveAssetName}
              onChange={e => setSaveAssetName(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleSaveAsset(); }}
              autoFocus
            />
            <div className="save-dialog-actions">
              <button className="btn-cancel" onClick={() => setShowSaveDialog(false)}>Cancel</button>
              <button className="btn-save" onClick={handleSaveAsset} disabled={!saveAssetName.trim()}>Save</button>
            </div>
          </div>
        </div>
      )}
